...
impl<I: Iterator<Item = u32>> Frontend<I> {
                    ...
                    *callee = fun_lookup.handle;
                    ...
                    for (arg_index, arg) in arguments.iter().enumerate() {
                        ...
                        match expressions[*arg] {
                            ...
                            // Array-indexed image: follow the base to find the global variable.
                            crate::Expression::Access { base, .. }
                            | crate::Expression::AccessIndex { base, .. } => {
                                let mut cur = base;
                                loop {
                                    match expressions[cur] {
                                        crate::Expression::GlobalVariable(handle) => {
                                            if let Some(sampling) = self.handle_sampling.get_mut(&handle) {
                                                *sampling |= flags;
                                            }
                                        ...
                                        crate::Expression::Access { base: inner, .. }
                                        | crate::Expression::AccessIndex { base: inner, .. } => {
                                            cur = inner;
                                        }
                                        _ => {
                                            log::warn!("Cannot resolve sampling flags for Access expression chain");
                                            break;
                                        }
                                    }
                                }
    ...
    pub fn parse(mut self) -> Result<crate::Module, Error> {
        ...
        // Collect handles that need splitting (both comparison and regular usage).
        let mut inconsistent_handles: Vec<Handle<crate::GlobalVariable>> = Vec::new();
        let mut changed_to_depth: Vec<Handle<crate::GlobalVariable>> = Vec::new();
        for (handle, flags) in self.handle_sampling.drain() {
            if !image::patch_comparison_type(
                ...
                inconsistent_handles.push(handle);
            }
            if flags.contains(image::SamplingFlags::COMPARISON) {
                changed_to_depth.push(handle);
            }
        }

        // After changing globals from Sampled(Float) to Depth, non-comparison
        // ImageSample returns f32 instead of vec4. Any AccessIndex { base: sample, index }
        // that was extracting .r/.g/.b/.a from the vec4 now fails (can't index f32).
        // Fix: replace those AccessIndex exprs with As { expr: base, kind: Float, convert: None }
        // which is a no-op bitcast (f32→f32) that validates and emits as `bitcast<f32>(...)`.
        //
        // IMPORTANT: Exclude inconsistent_handles (variables that will be split into
        // Depth + Float alias). Those variables' non-comparison ImageSample expressions
        // will be redirected to the Float alias by redirect_non_comparison_sampling,
        // where the result is vec4<f32> and AccessIndex works correctly. Applying
        // fix_depth_access_index to them would replace AccessIndex with As, which
        // then operates on vec4 after the redirect — causing InvalidStoreTypes.
        changed_to_depth.retain(|h| !inconsistent_handles.contains(h));
        if !changed_to_depth.is_empty() {
            for (_, fun) in module.functions.iter_mut() {
                Self::fix_depth_access_index(&changed_to_depth, &mut fun.expressions);
            }
            for ep in module.entry_points.iter_mut() {
                Self::fix_depth_access_index(&changed_to_depth, &mut ep.function.expressions);
            }
        }

        // Patch function parameter types: params with COMPARISON-only usage
        // (no REGULAR) → promote from Sampled(Float) to Depth.
        // Params with BOTH COMPARISON and REGULAR flags are left as Sampled(Float)
        // because they also do textureSampleGrad which requires texture_2d<f32>.
        // The function.rs ArgumentType relaxation allows passing Depth globals
        // to Sampled(Float) function parameters.
        for (_, lookup) in self.lookup_function.iter() {
            let fun = &mut module.functions[lookup.handle];
            for (i, &flags) in lookup.parameters_sampling.iter().enumerate() {
                if !flags.contains(image::SamplingFlags::COMPARISON) {
                    continue;
                }
                if flags.contains(image::SamplingFlags::REGULAR) {
                    continue; // Mixed — leave as Sampled(Float)
                }
                if i >= fun.arguments.len() {
                    continue;
                }
                let arg_ty = &module.types[fun.arguments[i].ty];
                if let crate::TypeInner::Image {
                    class: crate::ImageClass::Sampled { kind: crate::ScalarKind::Float, multi },
                    dim,
                    arrayed,
                } = arg_ty.inner {
                    let depth_ty = module.types.insert(
                        crate::Type {
                            name: None,
                    ...
                    fun.arguments[i].ty = depth_ty;
                }
            }
        }

        // For variables with both comparison and regular usage, split them:
        // - Keep original as Depth (comparison) type
        // - Create a clone as Float (non-comparison) type at binding+1
        // - Redirect all non-comparison ImageSample expressions to the clone
        for handle in inconsistent_handles {
            let var = &module.global_variables[handle];
            let var_ty = &module.types[var.ty];
            // Create the Depth (comparison) type for the original variable
            let (depth_ty, float_ty) = match var_ty.inner {
                crate::TypeInner::Image {
                    class: crate::ImageClass::Sampled { kind: crate::ScalarKind::Float, multi },
                    dim,
                    arrayed,
                    ...
                    (depth_ty, var.ty) // original type is already Float
                }
                crate::TypeInner::Image {
                    class: crate::ImageClass::Depth { multi },
                    dim,
                    ...
                    let float_ty = module.types.insert(
                        crate::Type {
                            name: None,
                            inner: crate::TypeInner::Image {
                    ...
                    (var.ty, float_ty) // original type is already Depth
                }
                _ => {
                    log::warn!("Cannot split inconsistent comparison variable: unexpected type");
                    continue;
                }
            };

            // Clone the variable for non-comparison usage at binding+1
            let clone_binding = var.binding.as_ref().map(|b| crate::ResourceBinding {
                group: b.group,
                binding: b.binding + 1,
            });
            let clone_var = crate::GlobalVariable {
                name: var.name.as_ref().map(|n| format!("{n}_depth_alias")),
                space: var.space,
                binding: clone_binding,
                ty: float_ty,
                init: None,
            };
            let clone_handle = module.global_variables.append(clone_var, crate::Span::UNDEFINED);

            // Set original to Depth type (for comparison sampling)
            module.global_variables[handle].ty = depth_ty;

            // Build a map from Function handle → parameters_sampling for call arg redirect
            let callee_param_sampling: FastHashMap<Handle<crate::Function>, Vec<image::SamplingFlags>> =
                self.lookup_function.values().map(|lf| {
                    (lf.handle, lf.parameters_sampling.clone())
                }).collect();

            // Walk all functions and entry points to redirect non-comparison
            // ImageSample/Call expressions from original → clone.
            for (_, fun) in module.functions.iter_mut() {
                Self::redirect_non_comparison_sampling(
                    handle, clone_handle, &mut fun.expressions,
                    &mut fun.body, &callee_param_sampling,
                );
            }
            for ep in module.entry_points.iter_mut() {
                Self::redirect_non_comparison_sampling(
                    handle, clone_handle, &mut ep.function.expressions,
                    &mut ep.function.body, &callee_param_sampling,
                );
            }

            // Function parameter types: mixed-usage params (both COMPARISON and REGULAR)
            // stay as Sampled(Float). The relaxed ArgumentType validation allows passing
            // a Depth image for a Sampled(Float) parameter at call sites. Inside the
            // function, comparison sampling on Sampled(Float) is handled by relaxing the
            // ComparisonSamplingMismatch check in the expression validator.

            if let Some(binding) = &module.global_variables[handle].binding {
                log::info!(
                    "Split inconsistent comparison variable @group({}) @binding({}) → depth + float alias @binding({})",
                    binding.group, binding.binding, binding.binding + 1
                );
            }
        }
    ...
    /// After `patch_comparison_type` changes a global's type from Sampled(Float) to Depth,
    /// non-comparison ImageSample on it returns f32 instead of vec4<f32>. Any
    /// `AccessIndex { base: image_sample, index }` that was extracting a component
    /// from the vec4 now fails because f32 isn't indexable.
    ///
    /// Fix: replace those AccessIndex expressions with
    /// `As { expr: base, kind: Float, convert: None }` — a no-op bitcast (f32→f32).
    ///
    /// Also fixes `SampleLevel::Exact(float_expr)` → `SampleLevel::Zero` on depth images,
    /// because WGSL requires integer LOD for depth textures but GLSL uses float.
    fn fix_depth_access_index(
        changed_to_depth: &[Handle<crate::GlobalVariable>],
        expressions: &mut Arena<crate::Expression>,
    ) {
        // First pass: find ImageSample handles that sample changed depth globals
        // without a depth_ref (non-comparison sampling).
        let depth_sample_handles: Vec<Handle<crate::Expression>> = expressions
            .iter()
            .filter_map(|(h, expr)| {
                if let crate::Expression::ImageSample { image, depth_ref: None, .. } = *expr {
                    // Check if image references a changed-to-depth global
                    match expressions[image] {
                        crate::Expression::GlobalVariable(gv) if changed_to_depth.contains(&gv) => {
                            Some(h)
                        }
                        _ => None,
                    }
        ...
        if depth_sample_handles.is_empty() {
            return;
        }

        // Second pass: fix AccessIndex and SampleLevel::Exact on depth samples.
        for (h, expr) in expressions.iter_mut() {
            match *expr {
                // Replace AccessIndex { base: depth_sample, index } with As { expr: base }
                crate::Expression::AccessIndex { base, .. } if depth_sample_handles.contains(&base) => {
                ...
                // Fix SampleLevel::Exact(float) → SampleLevel::Zero for depth textures.
                // WGSL requires integer LOD for depth textures; Zero is equivalent for LOD 0.
                crate::Expression::ImageSample {
                    ref mut level,
                    depth_ref: None,
                    ..
                } if depth_sample_handles.contains(&h) => {
                    if let crate::SampleLevel::Exact(_) = level {
    ...
    /// Redirect non-comparison `ImageSample` expressions and `Call` arguments
    /// from `orig_handle` to `clone_handle`.
    ///
    /// orig_handle's type has been set to Depth (comparison), clone_handle is Float (non-comparison).
    /// This method ensures:
    /// - Comparison ImageSample expressions use orig_handle (Depth)
    /// - Non-comparison ImageSample expressions use clone_handle (Float)
    /// - Call arguments to comparison-using function params use orig_handle (Depth)
    /// - Call arguments to non-comparison function params use clone_handle (Float)
    fn redirect_non_comparison_sampling(
        orig_handle: Handle<crate::GlobalVariable>,
        clone_handle: Handle<crate::GlobalVariable>,
        expressions: &mut Arena<crate::Expression>,
        body: &mut crate::Block,
        callee_param_sampling: &FastHashMap<Handle<crate::Function>, Vec<image::SamplingFlags>>,
    ) {
        // Collect all GlobalVariable expressions referencing orig_handle.
        let gv_exprs: Vec<Handle<crate::Expression>> = expressions
            .iter()
            .filter_map(|(h, expr)| match *expr {
                crate::Expression::GlobalVariable(gv) if gv == orig_handle => Some(h),
                _ => None,
            })
            .collect();

        if gv_exprs.is_empty() {
            return;
        }

        // Map each GV expression to its usage type from ImageSample expressions.
        // Each OpLoad in SPIR-V creates a distinct GV expression, so typically
        // each ImageSample references a unique GV expression.
        let mut gv_needs_clone: FastHashSet<Handle<crate::Expression>> = FastHashSet::default();
        let mut gv_needs_orig: FastHashSet<Handle<crate::Expression>> = FastHashSet::default();
        for (h, expr) in expressions.iter() {
            if let crate::Expression::ImageSample { image, depth_ref, .. } = *expr {
                if gv_exprs.contains(&image) {
                    if depth_ref.is_some() {
                        gv_needs_orig.insert(image); // comparison → keep as orig (Depth)
                    } else {
                        gv_needs_clone.insert(image); // non-comparison → redirect to clone (Float)
                    }
                }
            }
        }

        // Per-GV-expression redirect: change each GV expression in-place.
        // Since each ImageSample typically has its own GV expression (from
        // separate OpLoad instructions), this avoids appending new expressions
        // and eliminates ForwardDependency errors.
        for &gv_h in &gv_exprs {
            let needs_clone = gv_needs_clone.contains(&gv_h);
            let needs_orig = gv_needs_orig.contains(&gv_h);

            if needs_clone && !needs_orig {
                // Only non-comparison uses: redirect to clone
                *expressions.get_mut(gv_h) = crate::Expression::GlobalVariable(clone_handle);
            } else if needs_orig && needs_clone {
                // Shared by both types: redirect GV to clone (Float) for non-comparison,
                // and append a new GV for comparison users pointing to orig (Depth).
                // The appended GV may create a forward reference in the expression arena,
                // but the analyzer pre-populates GlobalVariable expression types to handle this.
                *expressions.get_mut(gv_h) = crate::Expression::GlobalVariable(clone_handle);
                let new_orig_gv = expressions.append(
                    crate::Expression::GlobalVariable(orig_handle),
                    crate::Span::UNDEFINED,
                );
                // Redirect comparison ImageSample expressions that used this GV
                for (h, expr) in expressions.iter_mut() {
                    if let crate::Expression::ImageSample { ref mut image, depth_ref, .. } = *expr {
                        if *image == gv_h && depth_ref.is_some() {
                            *image = new_orig_gv;
                        }
                    }
                }
            }
            // else: only comparison or no ImageSample use → keep pointing to orig
        }

        // Handle Call argument redirection. Statements→Expressions have no
        // ordering constraint, so we can freely use appended GV expressions.
        //
        // IMPORTANT: Phase A may have redirected GV expressions in-place to
        // clone_handle for non-comparison ImageSample uses. If those SAME GV
        // expressions are also used as Call arguments for comparison-targeting
        // functions (like sample_pcf_shadow), the Call arguments now incorrectly
        // point to the Float clone instead of the original Depth variable.
        // We must fix this by always redirecting comparison call args to the
        // original when there are comparison calls.
        let (has_comp_call, has_noncomp_call) =
            Self::scan_call_uses(body, &gv_exprs, callee_param_sampling);

        if has_noncomp_call {
            // Redirect non-comparison call args to the Float clone.
            let call_clone_gv = expressions.append(
                crate::Expression::GlobalVariable(clone_handle),
                crate::Span::UNDEFINED,
            );
            Self::redirect_call_args(body, &gv_exprs, call_clone_gv, false, callee_param_sampling);
        }
        if has_comp_call {
            // Redirect comparison call args to the original Depth variable.
            // This is critical: Phase A may have changed GV expressions in-place
            // to point to the clone, corrupting call arguments that should point
            // to the original Depth texture for comparison-sampling functions.
            let call_orig_gv = expressions.append(
                crate::Expression::GlobalVariable(orig_handle),
                crate::Span::UNDEFINED,
            );
            Self::redirect_call_args(body, &gv_exprs, call_orig_gv, true, callee_param_sampling);
        }
    }

    /// Recursively scan statements for Call arguments that reference the given GV
    /// expressions, returning (has_comparison, has_non_comparison).
    fn scan_call_uses(
        body: &crate::Block,
        gv_exprs: &[Handle<crate::Expression>],
        callee_param_sampling: &FastHashMap<Handle<crate::Function>, Vec<image::SamplingFlags>>,
    ) -> (bool, bool) {
        let mut comp = false;
        let mut noncomp = false;
        for stmt in body.iter() {
            match *stmt {
                crate::Statement::Call { function, ref arguments, .. } => {
                    if let Some(sampling) = callee_param_sampling.get(&function) {
                        for (i, &arg) in arguments.iter().enumerate() {
                            if gv_exprs.contains(&arg) {
                                if let Some(&flags) = sampling.get(i) {
                                    if flags.contains(image::SamplingFlags::COMPARISON) {
                                        comp = true;
                                    }
                                    if flags.contains(image::SamplingFlags::REGULAR) {
                ...
                crate::Statement::If { ref accept, ref reject, .. } => {
                    let (c, n) = Self::scan_call_uses(accept, gv_exprs, callee_param_sampling);
                    comp |= c; noncomp |= n;
                    let (c, n) = Self::scan_call_uses(reject, gv_exprs, callee_param_sampling);
                    comp |= c; noncomp |= n;
                }
                crate::Statement::Switch { ref cases, .. } => {
                    for case in cases {
                        let (c, n) = Self::scan_call_uses(&case.body, gv_exprs, callee_param_sampling);
                        comp |= c; noncomp |= n;
                    }
                }
                crate::Statement::Loop { ref body, ref continuing, .. } => {
                    let (c, n) = Self::scan_call_uses(body, gv_exprs, callee_param_sampling);
                    comp |= c; noncomp |= n;
                    let (c, n) = Self::scan_call_uses(continuing, gv_exprs, callee_param_sampling);
                    comp |= c; noncomp |= n;
                }
                _ => {}
            }
    ...
    /// Recursively walk statements and redirect Call arguments.
    /// If `redirect_comparison` is true, redirect args to comparison-using parameters to `new_gv`.
    /// If false, redirect args to non-comparison parameters to `new_gv`.
    fn redirect_call_args(
        body: &mut crate::Block,
        gv_exprs: &[Handle<crate::Expression>],
        new_gv: Handle<crate::Expression>,
        redirect_comparison: bool,
        callee_param_sampling: &FastHashMap<Handle<crate::Function>, Vec<image::SamplingFlags>>,
    ) {
        for stmt in body.iter_mut() {
            match *stmt {
                crate::Statement::Call { function, ref mut arguments, .. } => {
                    if let Some(sampling) = callee_param_sampling.get(&function) {
                        for (i, arg) in arguments.iter_mut().enumerate() {
                            if gv_exprs.contains(arg) {
                                if let Some(&flags) = sampling.get(i) {
                                    let is_comp = flags.contains(image::SamplingFlags::COMPARISON);
                                    if redirect_comparison && is_comp {
                                        *arg = new_gv;
                                    } else if !redirect_comparison && !is_comp {
                                        *arg = new_gv;
                                    }
                                }
                            }
                ...
                crate::Statement::If { ref mut accept, ref mut reject, .. } => {
                    Self::redirect_call_args(accept, gv_exprs, new_gv, redirect_comparison, callee_param_sampling);
                    Self::redirect_call_args(reject, gv_exprs, new_gv, redirect_comparison, callee_param_sampling);
                }
                crate::Statement::Switch { ref mut cases, .. } => {
                    for case in cases {
                        Self::redirect_call_args(&mut case.body, gv_exprs, new_gv, redirect_comparison, callee_param_sampling);
                    }
                }
                crate::Statement::Loop { ref mut body, ref mut continuing, .. } => {
                    Self::redirect_call_args(body, gv_exprs, new_gv, redirect_comparison, callee_param_sampling);
                    Self::redirect_call_args(continuing, gv_exprs, new_gv, redirect_comparison, callee_param_sampling);
                }
                _ => {}
            }
        }
