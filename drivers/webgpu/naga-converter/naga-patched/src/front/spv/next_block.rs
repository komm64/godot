            ...
            match inst.op {
                ...
                Op::ExtInst => {
                    ...
                    let fun = match gl_op {
                        ...
                        // Modf/Frexp old-style: return fract/sig and store whole/exp via pointer.
                        // We handle these specially below by converting to struct-returning variants.
                        Glo::Modf | Glo::Frexp => {
                            let mf = if gl_op == Glo::Modf { Mf::Modf } else { Mf::Frexp };

                            // Old-style Modf/Frexp: wc = 7 (result_type, result_id, set_id, inst_id, x, ptr)
                            inst.expect(base_wc + 2)?;

                            // Read the value argument
                            let x_id = self.next()?;
                            let x_lexp = self.lookup_expression.lookup(x_id)?;
                            let x_handle = get_expr_handle!(x_id, x_lexp);

                            // Read the pointer argument (where to store whole/exp)
                            let ptr_id = self.next()?;
                            let ptr_lexp = self.lookup_expression.lookup(ptr_id)?;
                            let ptr_handle = get_expr_handle!(ptr_id, ptr_lexp);

                            // Create the struct-returning version: Math { Modf/Frexp, x }
                            let struct_expr = crate::Expression::Math {
                                fun: mf,
                                arg: x_handle,
                                arg1: None,
                                arg2: None,
                            ...
                            let struct_handle = ctx.expressions.append(struct_expr, span);

                            // Extract field 0 (fract for Modf, sig for Frexp) — this is the return value
                            let fract_expr = crate::Expression::AccessIndex {
                                base: struct_handle,
                                index: 0,
                            };
                            let fract_handle = ctx.expressions.append(fract_expr, span);

                            // Extract field 1 (whole for Modf, exp for Frexp) — store to pointer
                            let whole_expr = crate::Expression::AccessIndex {
                                base: struct_handle,
                                index: 1,
                            };
                            let whole_handle = ctx.expressions.append(whole_expr, span);

                            // Emit the store to the pointer
                            block.extend(emitter.finish(ctx.expressions));
                            block.push(
                                crate::Statement::Store {
                                    pointer: ptr_handle,
                            ...
                            // Register the fractional part as the result of this instruction
                            self.lookup_expression.insert(
                                result_id,
                                LookupExpression {
                                    handle: fract_handle,
