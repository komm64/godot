        ...
        match *expression {
                ...
                match level {
                    ...
                    Sl::Zero => {
                        ...
                        if depth_ref.is_none() && !clamp_to_edge {
                            // WGSL requires integer LOD for depth textures, float for regular
                            let is_depth = matches!(
                                *func_ctx.resolve_type(image, &module.types),
                                TypeInner::Image { class: crate::ImageClass::Depth { .. }, .. }
                            );
                            if is_depth {
                                write!(self.out, ", 0")?;
                            } else {
            ...
            Expression::Relational { fun, argument } => {
                ...
                match fun {
                    ...
                    Rf::IsInf => {
                        // WGSL has no isInf(). Emulate: abs(x) > 3.402823466e+38
                        write!(self.out, "(abs(")?;
                        self.write_expr(module, argument, func_ctx)?;
                        write!(self.out, ") > 3.402823466e+38)")?;
                    }
                };
            }
            // Not supported yet
