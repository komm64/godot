...
impl ModuleInfo {
    ...
    pub(super) fn process_function(
        ...
        // Pre-populate types for GlobalVariable and FunctionArgument expressions.
        // Comparison-variable splitting may append GlobalVariable expressions after
        // the ImageSample expressions that reference them, creating forward references.
        // Pre-filling their types ensures the type resolver can handle them.
        for (handle, expr) in fun.expressions.iter() {
            match *expr {
                crate::Expression::GlobalVariable(gv) => {
                    let ty = module.global_variables[gv].ty;
                    info.expressions[handle.index()].ty = TypeResolution::Handle(ty);
                }
                crate::Expression::FunctionArgument(i) => {
                    if let Some(arg) = fun.arguments.get(i as usize) {
                        info.expressions[handle.index()].ty = TypeResolution::Handle(arg.ty);
                    }
                }
                _ => {}
            }
