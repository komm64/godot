    ...
    fn validate_call(
        ...
        for (index, (arg, &expr)) in fun.arguments.iter().zip(arguments).enumerate() {
            ...
            if !context.compare_types(&TypeResolution::Handle(arg.ty), ty) {
                // Allow Image class mismatches (Sampled{Float} vs Depth) for
                // SPIR-V depth=2 compatibility. In SPIR-V, images with
                // depth=2 are used for both comparison and non-comparison
                // sampling, which maps to both texture_2d<f32> and
                // texture_depth_2d in WGSL. Function parameters may have
                // either type depending on how the splitting resolved.
                let is_image_class_mismatch = {
                    let arg_inner = &context.types[arg.ty].inner;
                    let seen_inner = ty.inner_with(context.types);
                    matches!(
                        (arg_inner, seen_inner),
                        (
                            crate::TypeInner::Image { class: crate::ImageClass::Depth { .. }, .. },
                            crate::TypeInner::Image { class: crate::ImageClass::Sampled { kind: crate::ScalarKind::Float, .. }, .. }
                        ) | (
                            crate::TypeInner::Image { class: crate::ImageClass::Sampled { kind: crate::ScalarKind::Float, .. }, .. },
                            crate::TypeInner::Image { class: crate::ImageClass::Depth { .. }, .. }
                        )
