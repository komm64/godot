...
with open(test_header_path, "r", encoding="utf-8") as f:
    content = f.read()
...
if os.path.exists(gen_h_path):
    with open(gen_h_path, "r", encoding="utf-8") as f:
        content = f.read()
...
# =========================================================================
# Test 9: Host-tool environment isolation
# =========================================================================
print("\n=== Test 9: Host-tool environment isolation ===")
...
target_toolchain = {
    ...
    "LD": "wasm-ld",
    "RANLIB": "emranlib",
    "KEEP_ME": "host-setting",
}
with patch.dict(os.environ, target_toolchain, clear=True):
    host_env = wgsl_precompile._host_tool_environment()

for variable in wgsl_precompile._HOST_TOOLCHAIN_ENV_VARS:
    assert_true(variable not in host_env, f"Target {variable} is removed")
assert_eq(host_env.get("KEEP_ME"), "host-setting",
          "Unrelated environment variables are preserved")

posix_path = wgsl_precompile._relative_posix_path(
    os.path.join("repo", "drivers", "webgpu", "wgsl_precompile.py"),
    "repo",
)
